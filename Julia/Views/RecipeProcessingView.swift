...
struct RecipeProcessingView: View {
  ...
  @State private var showingReconstructedText = false
            ...
            Menu {
              Button(showingRawText ? "Hide Raw Text" : "Show Raw Text") {
                withAnimation {
                  ...
                  if showingRawText {
                    showingReconstructedText = false
                  }
                }
              }
              
              Button(showingReconstructedText ? "Hide Reconstructed Text" : "Show Reconstructed Text") {
                withAnimation {
                  showingReconstructedText.toggle()
                  if showingReconstructedText {
  ...
  private var reconstructedTextDebugView: some View {
    VStack {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 8) {
          ForEach(reconstructedText, id: \.self) { line in
            Text(line)
              .font(.system(.body, design: .monospaced))
          }
        }
  ...
  private func classifyText() {
    ...
    Task {
      ...
      let processed = await classifier.processRecipeTextAsync(reconstructed)
  ...
  private func reconstructTextAsync(_ lines: [String]) async -> [String] {
    return await withCheckedContinuation { continuation in
      DispatchQueue.global().async {
        let reconstructed = RecipeTextReconstructor.prepareForClassification(from: lines)
