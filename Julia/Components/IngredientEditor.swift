...
struct IngredientEditor: View {
    ...
    private var isAnyFieldFocused: Bool {
        return isNameFieldFocused || isCommentFieldFocused
    }
    ...
    var body: some View {
      VStack(spacing: 8) {
        ...
        VStack(alignment: .center, spacing: 12) {
          ...
          // Name field and focus change tracking
          Group {
          ...
          } // End of animated VStack
          .frame(height: isAnyFieldFocused ? 0 : nil, alignment: .top)
          .opacity(isAnyFieldFocused ? 0 : 1)
            ...
            if !isNameFieldFocused {
              TextField("Comment (e.g., diced, chopped)", text: $comment)
                ...
                .onSubmit {
                  ...
                  // Wait a short time before saving, so field blur animation completes
      ...
      .onChange(of: isNameFieldFocused) { oldValue, newValue in
        ...
        if oldValue && !newValue && name.contains(" ") {
            ...
            if let parsedIngredient = IngredientParser.fromString(input: name, location: ingredientLocation) {
                ...
                comment = parsedIngredient.comment ?? ""
            }
        }
      }
      .animation(.spring(response: 0.35, dampingFraction: 0.8), value: isNameFieldFocused)
      .animation(.spring(response: 0.35, dampingFraction: 0.8), value: isAnyFieldFocused)
    }
...
#Preview {
    ...
    struct PreviewWrapper: View {
        ...
        @State private var location: IngredientLocation = .recipe
